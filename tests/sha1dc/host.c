...
#include "control.h"
#include "sha1dc.skel.h"
int main(void) {
    struct sha1dc *s = sha1dc__open();
    ...
    if (bpf_capsule_configure(
            &capsule, s->obj,
            (struct bpf_capsule_config){.fiber_count = 1, .heap_bytes = 16ULL << 20}) ||
        ...
        goto done;
    volatile struct sha1dc_state *state = &s->data_test->sha1dc_state;
    int fd = bpf_program__fd(s->progs.sha1dc_start);
    for (unsigned i = 0; i < 100000; i++) {
        struct bpf_test_run_opts opt = {.sz = sizeof(opt)};
        if (bpf_prog_test_run_opts(fd, &opt)) {
            perror("run");
            goto done;
        }
        ...
        fd = bpf_program__fd(s->progs.sha1dc_continue);
    }
    fprintf(stderr, "SHA1DC test exceeded continuation limit\n");
done:
