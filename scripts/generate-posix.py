...
guest='/* Generated by scripts/generate-posix.py. */\n'
host=guest+'static int64_t service_posix(struct bridge_host *h, struct posix_request *request) {\n    uint64_t *a=request->args;\n    switch(request->operation) {\n'
...
for ordinal,(name,result,parameters) in enumerate(functions,1):
    ...
    if name in {'setuid','setgid','setresuid','setresgid'}:
    ...
    for index,(t,n,k) in enumerate(parameters):
        ...
        elif k=='atflags':host+=f'        int {n}=at_flags(a[{index}]);\n'
