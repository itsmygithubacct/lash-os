...
#include "_mbedtls_pk_private.h"
#include "_mbedtls_pk_internal.h"
#include "_mbedtls_rsa.h"
#include "_mbedtls_rsa_internal.h"
#include "_mbedtls_asn1.h"
#include "_mbedtls_crypto.h"
#include "_mbedtls_psa_crypto_rsa.h"
...
#define BSSH_MBED4_RSA_DER_MAX 16384

static int bssh_mbed4_rsa_complete(mbedtls_rsa_context *rsa)
{
    int rc;
    mbedtls_mpi p1;
    mbedtls_mpi q1;
    ...
    rc = mbedtls_mpi_sub_int(&p1, &rsa->P, 1);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_sub_int(&q1, &rsa->Q, 1);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_mod_mpi(&rsa->DP, &rsa->D, &p1);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_mod_mpi(&rsa->DQ, &rsa->D, &q1);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_inv_mod(&rsa->QP, &rsa->Q, &rsa->P);
...
static int bssh_mbed4_rsa_public_der_from_mpints(ssh_string e,
                                                 ssh_string n,
                                                 unsigned char *der,
                                                 size_t der_size,
                                                 size_t *der_len)
{
    ...
    rc = mbedtls_mpi_read_binary(&rsa.N, ssh_string_data(n), ssh_string_len(n));
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_read_binary(&rsa.E, ssh_string_data(e), ssh_string_len(e));
    if (rc != 0) {
        goto out;
    }
    rsa.len = (mbedtls_mpi_bitlen(&rsa.N) + 7) >> 3;
    rc = (int)mbedtls_psa_rsa_export_key(PSA_KEY_TYPE_RSA_PUBLIC_KEY,
                                         &rsa,
                                         der,
                                         der_size,
                                         der_len);
out:
    ...
    return rc == PSA_SUCCESS ? 0 : rc;
}

static int bssh_mbed4_rsa_private_der_from_mpints(ssh_string n,
                                                  ssh_string e,
                                                  ssh_string d,
                                                  ssh_string p,
                                                  ssh_string q,
                                                  ...
                                                  size_t *der_len)
{
    ...
    rc = mbedtls_mpi_read_binary(&rsa.D, ssh_string_data(d), ssh_string_len(d));
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_read_binary(&rsa.P, ssh_string_data(p), ssh_string_len(p));
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_mpi_read_binary(&rsa.Q, ssh_string_data(q), ssh_string_len(q));
    if (rc != 0) {
        goto out;
    }
    rc = bssh_mbed4_rsa_complete(&rsa);
    if (rc != 0) {
        goto out;
    }
    ...
    rc = (int)mbedtls_psa_rsa_export_key(PSA_KEY_TYPE_RSA_KEY_PAIR,
                                         &rsa,
                                         der,
                                         der_size,
                                         der_len);
...
static int bssh_mbed4_pk_setup_rsa_der(mbedtls_pk_context *pk,
                                       ...
                                       int private_key)
{
    ...
    return mbedtls_pk_rsa_set_pubkey(pk, der, der_len);
}

static int bssh_mbed4_asn1_get_rsa_components(const unsigned char *der,
                                              size_t der_len,
                                              int private_key,
                                              mbedtls_mpi *n,
                                              mbedtls_mpi *e,
                                              ...
                                              mbedtls_mpi *iqmp)
{
    ...
    size_t len;
    mbedtls_mpi ver;
    unsigned char *cur = (unsigned char *)der;
    const unsigned char *end = der + der_len;
    const unsigned char *seq_end;
    mbedtls_mpi dp;
    mbedtls_mpi dq;

    mbedtls_mpi_init(&ver);
    mbedtls_mpi_init(&dp);
    mbedtls_mpi_init(&dq);

    rc = mbedtls_asn1_get_tag(&cur, end, &len,
    ...
    if (private_key) {
        rc = mbedtls_asn1_get_mpi(&cur, seq_end, &ver);
        if (rc != 0) {
            goto out;
        }
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, n);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, e);
    if (rc != 0 || !private_key) {
        goto out;
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, d);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, p);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, q);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, &dp);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_asn1_get_mpi(&cur, seq_end, &dq);
    if (rc != 0) {
        goto out;
    }
    if (iqmp != NULL) {
        rc = mbedtls_asn1_get_mpi(&cur, seq_end, iqmp);
    }

out:
    mbedtls_mpi_free(&ver);
    mbedtls_mpi_free(&dp);
    mbedtls_mpi_free(&dq);
    return rc;
}

static int bssh_mbed4_export_rsa_components(mbedtls_pk_context *pk,
                                            int private_key,
                                            mbedtls_mpi *n,
                                            mbedtls_mpi *e,
                                            mbedtls_mpi *d,
                                            ...
                                            mbedtls_mpi *iqmp)
{
    ...
    unsigned char der[BSSH_MBED4_RSA_DER_MAX];
    size_t der_len = 0;

    if (private_key) {
        status = psa_export_key(pk->priv_id, der, sizeof(der), &der_len);
        if (status != PSA_SUCCESS) {
            return (int)status;
        }
    } else if (pk->pub_raw_len != 0) {
        memcpy(der, pk->pub_raw, pk->pub_raw_len);
        der_len = pk->pub_raw_len;
    } else {
        status = psa_export_public_key(pk->priv_id, der, sizeof(der), &der_len);
        if (status != PSA_SUCCESS) {
            return (int)status;
        }
    }

    return bssh_mbed4_asn1_get_rsa_components(der, der_len, private_key,
                                              n, e, d, p, q, iqmp);
}

static int bssh_mbed4_ecdsa_from_pk(mbedtls_ecdsa_context *ecdsa,
                                    mbedtls_pk_context *pk)
{
    int rc;
    unsigned char d[MAX_KEY_SIZE];
    size_t d_len = 0;
    mbedtls_ecp_group_id gid;
    mbedtls_ecp_keypair keypair;

    mbedtls_ecp_keypair_init(&keypair);
    gid = mbedtls_ecc_group_from_psa(pk->ec_family, pk->bits);
    rc = mbedtls_ecp_group_load(&keypair.grp, gid);
    if (rc != 0) {
        goto out;
    }
    rc = mbedtls_ecp_point_read_binary(&keypair.grp,
                                       &keypair.Q,
                                       pk->pub_raw,
                                       pk->pub_raw_len);
    if (rc != 0) {
    ...
    {
        psa_status_t status = psa_export_key(pk->priv_id, d, sizeof(d), &d_len);
        if (status == PSA_SUCCESS) {
            rc = mbedtls_mpi_read_binary(&keypair.d, d, d_len);
            if (rc != 0) {
                goto out;
            }
        }
    }
    rc = mbedtls_ecdsa_from_keypair(ecdsa, &keypair);
...
#if MBEDTLS_VERSION_MAJOR >= 4
        valid = bssh_mbed4_ecdsa_from_pk(key->ecdsa, pk);
#else
        {
            mbedtls_ecp_keypair *keypair = mbedtls_pk_ec(*pk);
            valid = mbedtls_ecdsa_from_keypair(key->ecdsa, keypair);
        }
#endif
        if (valid != 0) {
            goto fail;
                          ...
                          ssh_string q)
{
    ...
    rc = bssh_mbed4_rsa_private_der_from_mpints(n, e, d, p, q,
                                                der, sizeof(der), &der_len);
    if (rc == 0) {
        rc = bssh_mbed4_pk_setup_rsa_der(key->pk, der, der_len, 1);
    }
    ssh_burn(der, sizeof(der));
    if (rc != 0) {
        mbedtls_pk_free(key->pk);
        SAFE_FREE(key->pk);
        return SSH_ERROR;
...
int pki_pubkey_build_rsa(ssh_key key, ssh_string e, ssh_string n)
{
    ...
    rc = bssh_mbed4_rsa_public_der_from_mpints(e, n, der, sizeof(der), &der_len);
    if (rc == 0) {
        rc = bssh_mbed4_pk_setup_rsa_der(key->pk, der, der_len, 0);
    }
    ssh_burn(der, sizeof(der));
    if (rc != 0) {
        mbedtls_pk_free(key->pk);
    ...
    switch(key->type) {
        case SSH_KEYTYPE_RSA:
            ...
            if (!demote && (key->flags & SSH_KEY_FLAG_PRIVATE)) {
                rc = mbedtls_pk_copy_from_psa(key->pk->priv_id, new->pk);
            } else if (key->pk->pub_raw_len != 0) {
                rc = bssh_mbed4_pk_setup_rsa_der(new->pk,
                                                 key->pk->pub_raw,
                                                 key->pk->pub_raw_len,
                                                 0);
            } else {
                rc = mbedtls_pk_copy_public_from_psa(key->pk->priv_id, new->pk);
            }
            if (rc != 0) {
                goto fail;
            }
#else
            {
                ...
                rc = mbedtls_rsa_copy(new_rsa, rsa);
                if (rc != 0) {
                    goto fail;
                }
            }
        ...
        case SSH_KEYTYPE_SK_ECDSA:
            ...
            if ((demote && ssh_key_is_private(key)) ||
                ...
                rc = mbedtls_ecdsa_from_keypair(new->ecdsa, key->ecdsa);
                if (rc != 0) {
                    goto fail;
                }
            }
...
int pki_key_generate_rsa(ssh_key key, int parameter)
{
    ...
    if (rc == 0 && mbedtls_pk_can_do(key->pk, MBEDTLS_PK_RSA)) {
...
{
    int rc = 0;
    mbedtls_mpi N1, N2, P1, P2, Q1, Q2, E1, E2;
    ...
    switch (ssh_key_type_plain(k1->type)) {
        case SSH_KEYTYPE_RSA:
            if (!mbedtls_pk_can_do(k1->pk, MBEDTLS_PK_RSA) ||
                ...
                mbedtls_pk_get_type(k1->pk) != mbedtls_pk_get_type(k2->pk) ||
                mbedtls_pk_get_bitlen(k1->pk) != mbedtls_pk_get_bitlen(k2->pk)) {
                rc = 1;
                goto cleanup;
            }
#if MBEDTLS_VERSION_MAJOR >= 4
            rc = bssh_mbed4_export_rsa_components(k1->pk,
                                                  what == SSH_KEY_CMP_PRIVATE,
                                                  &N1, &E1,
                                                  NULL, &P1, &Q1, NULL);
            if (rc != 0) {
                rc = 1;
            ...
            rc = bssh_mbed4_export_rsa_components(k2->pk,
                                                  what == SSH_KEY_CMP_PRIVATE,
                                                  &N2, &E2,
                                                  NULL, &P2, &Q2, NULL);
            if (rc != 0) {
...
#else
            rc = mbedtls_rsa_export(mbedtls_pk_rsa(*k1->pk), &N1, &P1, &Q1, NULL, &E1);
            if (rc != 0) {
                rc = 1;
                goto cleanup;
            }
            rc = mbedtls_rsa_export(mbedtls_pk_rsa(*k2->pk), &N2, &P2, &Q2, NULL, &E2);
            if (rc != 0) {
                rc = 1;
                goto cleanup;
            }
            ...
            if (mbedtls_mpi_cmp_mpi(&N1, &N2) != 0 ||
                mbedtls_mpi_cmp_mpi(&E1, &E2) != 0 ||
                (what == SSH_KEY_CMP_PRIVATE &&
                 (mbedtls_mpi_cmp_mpi(&P1, &P2) != 0 ||
                  mbedtls_mpi_cmp_mpi(&Q1, &Q2) != 0))) {
                rc = 1;
            }
            break;
        case SSH_KEYTYPE_ECDSA_P256:
        ...
        case SSH_KEYTYPE_SK_ECDSA: {
            ...
            if (ecdsa1->MBEDTLS_PRIVATE(grp).id !=
                ...
                mbedtls_mpi_cmp_mpi(&ecdsa1->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(X),
                                    &ecdsa2->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(X)) ||
                mbedtls_mpi_cmp_mpi(&ecdsa1->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Y),
                                    &ecdsa2->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Y)) ||
                mbedtls_mpi_cmp_mpi(&ecdsa1->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Z),
            ...
            if (what == SSH_KEY_CMP_PRIVATE &&
                ...
                mbedtls_mpi_cmp_mpi(&ecdsa1->MBEDTLS_PRIVATE(d),
...
#if MBEDTLS_VERSION_MAJOR >= 4
        rc = bssh_mbed4_export_rsa_components(key->pk, 0,
                                              &N, &E, NULL, NULL, NULL, NULL);
        if (rc != 0) {
            goto out;
        }
        E_ptr = &E;
...
#if MBEDTLS_VERSION_MAJOR >= 4
            rc = bssh_mbed4_export_rsa_components(key->pk, 1,
                                                  &N, &E, &D, &P, &Q, &IQMP);
            if (rc != 0) {
                goto out;
            }
