 ...
 */
static int seterror_at(ebuf_t ebuf, int lineno, int colno, const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  char *p = ebuf.ptr;
  char *q = p + ebuf.len;
  if (lineno) {
    if (colno > 0) {
      snprintf(p, p < q ? q - p : 0, "(line %d, column %d) ", lineno,
               colno);
    } else {
...
#define SETERROR(ebuf, lineno, ...) seterror_at((ebuf), (lineno), 0, __VA_ARGS__)
#define SETERRORC(ebuf, lineno, colno, ...) \
  seterror_at((ebuf), (lineno), (colno), __VA_ARGS__)
...
toml_result_t toml_parse(const char *src, int len) {
  ...
  for (;;) {
    ...
    SETERRORC(pp->ebuf, tok.lineno, tok.colno, "ENDL expected");
    goto bail;
  }
...
static int parse_key(parser_t *pp, token_t tok, keypart_t *ret_keypart) {
  ...
  if (tok.toktyp != TOK_STRING && tok.toktyp != TOK_LITSTRING &&
      tok.toktyp != TOK_LIT) {
    return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "missing key");
  }
  ...
  if (parse_norm(pp, tok, &kpspan[n])) {
    return SETERRORC(pp->ebuf, tok.lineno, tok.colno,
  ...
  while (1) {
    ...
    if (n >= KEYPARTMAX) {
      return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "too many key parts");
    }
  ...
  for (int i = 0; i < keypart->nspan; i++) {
    ...
    if (j < 0) {
      ...
      if (tab_add(tab, keypart->span[i], newtab, &reason)) {
        SETERRORC(pp->ebuf, lineno, colno, "%s", reason);
        return NULL;
      }
    ...
    if (value->type == TOML_ARRAY) {
      ...
      if (value->u.arr.size <= 0) {
        SETERRORC(pp->ebuf, lineno, colno, "array %s has no elements",
                  keypart->span[i].ptr);
        return NULL;
      }
      ...
      // It must be a table!
      if (value->type != TOML_TABLE) {
        SETERRORC(pp->ebuf, lineno, colno,
                  "array %s must be array of tables", keypart->span[i].ptr);
        return NULL;
      }
      tab = value; // descend
      continue;
    ...
    // key not found
    SETERRORC(pp->ebuf, lineno, colno, "cannot locate table at key %s",
              keypart->span[i].ptr);
    return NULL;
  }
  ...
  *ret_datum = mkdatum_at(TOML_TABLE, tok.lineno, tok.colno);
  ...
  for (;;) {
    ...
    if (tok.toktyp == TOK_COMMA) {
      ...
      return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "unexpected comma");
    }

    // Python tomllib targets TOML 1.0, where inline tables are single-line.
    if (tok.toktyp == TOK_ENDL) {
      return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "unexpected newline");
    }

    // Not a comma, but need a comma: error!
    if (need_comma) {
      return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "missing comma");
    }
    ...
    if (tok.toktyp != TOK_EQUAL) {
      if (tok.toktyp == TOK_ENDL) {
        ...
        return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "missing '='");
      }
    }
    ...
    if (tab_add(tab, lastkeypart, value, &reason)) {
      datum_free(&value);
      return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "%s", reason);
    }
...
static int parse_val(parser_t *pp, token_t tok, toml_datum_t *ret) {
  *ret = DATUM_ZERO; // initialize
  ...
  default:
    return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "missing value");
  }
}
...
static int parse_std_table_expr(parser_t *pp, token_t tok) {
  ...
  DO(scan_key(&pp->scanner, &tok));
  if (tok.toktyp != TOK_RBRACK) {
    return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "missing right-bracket");
  }
  ...
  if (j < 0) {
    // If not found: add it.
    if (tab->flag & FLAG_INLINED) {
      return SETERRORC(pp->ebuf, keylineno, keycolno,
                       "inline table cannot be extended");
    }
    const char *reason;
    ...
    if (tab_add(tab, lastkeypart, newtab, &reason)) {
      return SETERRORC(pp->ebuf, keylineno, keycolno, "%s", reason);
    }
    // this is the new tab
...
static int parse_array_table_expr(parser_t *pp, token_t tok) {
  ...
  if (rrb.toktyp != TOK_RRBRACK) {
    return SETERRORC(pp->ebuf, rrb.lineno, rrb.colno, "missing ']]'");
  }
  ...
  for (int i = 0; i < keypart.nspan; i++) {
    ...
    if (value->type == TOML_ARRAY) {
      if (value->flag & FLAG_INLINED) {
        return SETERRORC(pp->ebuf, keylineno, keycolno,
                         "cannot expand array %s", curkey.ptr);
      }
      if (value->u.arr.size <= 0) {
        return SETERRORC(pp->ebuf, keylineno, keycolno,
                         "array %s has no elements", curkey.ptr);
      }
      value = &value->u.arr.elem[value->u.arr.size - 1];
      if (value->type != TOML_TABLE) {
        return SETERRORC(pp->ebuf, keylineno, keycolno,
    ...
    // keypart not found
    return SETERRORC(pp->ebuf, keylineno, keycolno,
                     "cannot locate table at key %s", curkey.ptr);
  }
...
static int parse_keyvalue_expr(parser_t *pp, token_t tok) {
  ...
  DO(scan_key(&pp->scanner, &tok));
  if (tok.toktyp != TOK_EQUAL) {
    return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "expect '='");
  }
  ...
  for (int i = 0; i < keypart.nspan - 1; i++) {
    ...
    return SETERRORC(pp->ebuf, keylineno, keycolno,
                     "cannot locate table at '%s'", keypart.span[i].ptr);
  }
...
static int parse_norm(parser_t *pp, token_t tok, span_t *ret_span) {
  ...
  if (!p) {
    return SETERRORC(pp->ebuf, tok.lineno, tok.colno, "out of memory");
  }
