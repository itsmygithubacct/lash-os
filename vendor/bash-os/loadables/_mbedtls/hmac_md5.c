/* _mbedtls/hmac_md5.c — HMAC-MD5 (Stage 19.B). */
#include "hmac_md5.h"
#include <string.h>

#define BLOCK MBEDTLS_HMAC_MD5_BLOCK_SIZE
#define MD    MBEDTLS_HMAC_MD5_DIGEST_SIZE

void mbedtls_hmac_md5_init(mbedtls_hmac_md5_context *ctx)
{
    mbedtls_md5_init(&ctx->inner);
                            ...
                            const unsigned char *key, size_t keylen)
{
    ...
    if (keylen > BLOCK) {
        if ((ret = mbedtls_md5(key, keylen, k)) != 0) return ret;
    ...
    if ((ret = mbedtls_md5_starts(&ctx->inner)) != 0) goto out;
    if ((ret = mbedtls_md5_update(&ctx->inner, k_ipad, BLOCK)) != 0) goto out;
out:
    {
...
{
    return mbedtls_md5_update(&ctx->inner, input, ilen);
}
                            ...
                            unsigned char *output)
{
    ...
    int ret;
    if ((ret = mbedtls_md5_finish(&ctx->inner, inner_digest)) != 0) return ret;
    if ((ret = mbedtls_md5_starts(&ctx->inner)) != 0) goto out;
    if ((ret = mbedtls_md5_update(&ctx->inner, ctx->k_opad, BLOCK)) != 0) goto out;
    if ((ret = mbedtls_md5_update(&ctx->inner, inner_digest, MD)) != 0) goto out;
    ret = mbedtls_md5_finish(&ctx->inner, output);
out:
    {
                     ...
                     unsigned char *output)
{
    ...
    if ((ret = mbedtls_hmac_md5_starts(&ctx, key, keylen)) != 0) goto out;
    if ((ret = mbedtls_hmac_md5_update(&ctx, input, ilen)) != 0) goto out;
    ret = mbedtls_hmac_md5_finish(&ctx, output);
out:
